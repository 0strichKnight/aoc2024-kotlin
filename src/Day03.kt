 ...
 */
fun main() {
    fun extractMulNumbers(string: String): List<Int> {
        return string.split(",")
            .map { it.toInt() }
    }

    fun parseTokens(token: String, tokenId: String, input: String): Map<Int, String> {
        val tokens = mutableMapOf<Int, String>()
        var from = input.indexOf(token)

        while (from != -1 && from < input.length - 1) {
            tokens[from] = tokenId
            from = input.indexOf(token, from + 1)
        }

        return tokens
    }

    fun parseMul(input: String): Map<Int, String> {
        val tokens = mutableMapOf<Int, String>()
        var from = input.indexOf("mul(")

        while (from != -1 && from < input.length - 1) {
            val to = input.indexOf(")", from)
            try {
                extractMulNumbers(input.substring(from + 4, to))
                tokens[from] = input.substring(from + 4, to)
            } catch (_: Exception) {
                // ignore
            }

            from = input.indexOf("mul(", from + 4)
        }
    ...
    fun part1(input: String): Int {
        var total = 0
        val tokens = parseMul(input)

        tokens.toSortedMap().values.forEach {
            val numbers = extractMulNumbers(it)
            total += numbers[0].times(numbers[1])
        }
    ...
    fun part2(input: String): Int  {
        ...
        tokens.putAll(parseMul(input))
        tokens.putAll(parseTokens("do()", "do", input))
        tokens.putAll(parseTokens("don't()", "dont", input))
