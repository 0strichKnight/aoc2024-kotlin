...
fun main() {
    ...
    fun getLocationLists(input: List<String>): Pair<MutableList<Int>, MutableList<Int>> {
        val left = mutableListOf<Int>()
        val right = mutableListOf<Int>()
        input.forEach { line ->
            val list = line.trim().split(" ").filter { it.isNotBlank() }
            val leftNumber = list[0].trim().toInt()
            val rightNumber = list[1].trim().toInt()
            left.add(leftNumber)
    ...
    fun countRightSideOccurrences(numbers: List<Int>): Map<Int, Int> {
        val rightTimes = mutableMapOf<Int, Int>()

        numbers.forEach {
            rightTimes[it] = rightTimes.getOrDefault(it, 0) + 1
        }
    ...
    fun part1(input: List<String>): Int {
        val pairOfLists = getLocationLists(input)

        var distances = 0
        for (i in input.indices) {
            distances += (pairOfLists.first[i] - pairOfLists.second[i]).absoluteValue
        }
        return distances
    }
    ...
    fun part2(input: List<String>): Int {
        val pairOfLists = getLocationLists(input)
        val rightTimes = countRightSideOccurrences(pairOfLists.second)

        var similarities = 0
        for (number in pairOfLists.first) {
            similarities += number * rightTimes.getOrDefault(number, 0)
        }
