...
fun main() {
    ...
    fun isReportSafe(report: List<Int>): Boolean {
        ...
        for (i in 1..< report.size) {
            if (asc && report[i] < report[i-1] || !asc && report[i] > report[i-1]) {
                safe = false
            } else if ((report[i] - report[i-1]).absoluteValue !in 1..3) {
                safe = false
            }
    ...
    fun part1(input: List<String>): Int {
        var safeReports = 0
        input.forEach { reportLine ->
            val report: MutableList<Int> =
                reportLine.split(" ")
                    .map { it.toInt() }
                    .toMutableList()
        ...
        input.forEach { reportLine ->
            ...
            if (isReportSafe(report)) {
                ...
                for (i in report.indices) {
                    if (isReportSafe(report.filterIndexed { index, _ -> index != i })) {
                        safeReports += 1
                        break
                    }
                }
