...
fun main() {
    ...
    fun parseInput(input: List<String>): Pair<List<String>, List<String>> {
        ...
        var populatingRules = true

        input.forEach {
            if (it.isBlank()) {
                populatingRules = false
            } else {
                if (populatingRules) rules.add(it)
                else updates.add(it)
            }
        }
    ...
    fun compare(a: Int?, b: Int?, rules: List<String>): Int {
        rules.forEach { rule ->
            val ruleNumbers = rule.split("|").map { it.toInt() }

            if (a == ruleNumbers[0] && b == ruleNumbers[1]) {
                return -1
            } else if (a == ruleNumbers[1] && b == ruleNumbers[0]) {
                return 1
            }
        }
    ...
    fun separateValidInvalidUpdates(rules: List<String>, updates: List<String>):
        ...
        updates.forEach { update ->
            ...
            val pageComparator = Comparator<Int> { o1, o2 -> compare(o1, o2, rules) }

            if (updatePages == updatePages.sortedWith(pageComparator)) {
                valid.add(updatePages)
            } else {
                invalid.add(updatePages)
            }
        }
    ...
    fun part1(input: List<String>): Int {
        ...
        var totalValid = 0
        separateValidInvalidUpdates(parsedInput.first, parsedInput.second)
            .first
            .forEach { update ->
                val middle = update.size / 2
                totalValid += update[middle]
            }
    ...
    fun part2(input: List<String>): Int  {
        val parsedInput = parseInput(input)
        val pageComparator = Comparator<Int> { o1, o2 -> compare(o1, o2, parsedInput.first) }
        ...
        separateValidInvalidUpdates(parsedInput.first, parsedInput.second)
            ...
            .forEach { update ->
                val middle = update.size / 2
                totalInvalid += update.sortedWith(pageComparator)[middle]
            }
